...
namespace CM3D2.Maidirector.Plugin
{
        ...
        override public void OnGUI()
        {
            try
            {
                ...
                if( this._items != null && this._isShowDropDownList )
                {
                    ...
                    GlobalComboBox.Set(listRect, itemHeight, contents, this.FontSize, pick);
                }
            }
            catch( Exception e )
            {
        ...
        override public void OnGUI()
        {
            ...
            if( GUI.Button( curveRect, string.Empty, labelStyle ) )
            {
                GlobalCurveWindow.Set(new Vector2(this.Left + this.ScreenPos.x, this.Top + this.ScreenPos.y), this.FontSize * 15, this.FontSize, this._value,
                        ...
                        {
                            this._value = x;
                            Texture2D.Destroy(this._curveTex);
    ...
    internal class CustomDragPoint: ControlBase
    {
        public CustomDragPoint()
        {
            try
            {
                _value = new Vector2(30, 30);
            }
            catch (Exception e)
            {
                Debug.LogError(e.ToString());
        ...
        override public void OnGUI()
        {
            Rect boundRect = new Rect(this.Left, this.Top, this.Width, this.Height);

            Rect pointRect = new Rect(this._value.x - 7, this._value.y - 7, 15, 15);
